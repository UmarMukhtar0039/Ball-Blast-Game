...
function particleSystem:update(dt,worldTranslation)
	...
	if((not self.isPaused or self.forceSingleEmission) and particleSystem.allowParticles)then
		...
		if(self.emissionTimer>self.emissionTimeLimit)then
			...
			for i=1, self.count do
				...
				if(#self.particlePath>0)then
					...
					self.imageTable[#self.imageTable+1]=display.newImage(self.displayGroup,imagePath,x,y)
				else
					self.imageTable[#self.imageTable+1]=display.newImage(self.displayGroup,self.particlePath,x,y)
				end
		...
		if(self.forceSingleEmission)then
			self.forceSingleEmission=false-- always turn off the single emission flag after 1 successful emission
